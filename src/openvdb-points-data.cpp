...
OpenVDBPointsData::OpenVDBPointsData(const char *filename, const char *gridName, LoggingCallback cb)
{
    try
    {
...
openvdb::Index64 OpenVDBPointsData::pointCount()
{
    openvdb::Index64 count = openvdb::points::pointCount(gridPtr->tree());
    return count;
}

openvdb::Index64 OpenVDBPointsData::populatePoints(openvdb::math::Mat4s worldToClipMatrix, bool frustumCulling, bool lod, Point *points, LoggingCallback cb)
{
    ...
    return i;
        ...
        // cb("wrote vertices"http://xdpixel.com/native-rendering-plugin-in-unity/